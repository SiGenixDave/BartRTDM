...
static void OutputStream (TYPE_RTDM_STREAM_IF *interface, BOOL networkAvailable,
...
static UINT16 Check_Fault (UINT16 error_code);
...
void RTDM_Stream (TYPE_RTDM_STREAM_IF *interface, RtdmXmlStr *rtdmXmlData)
{
    ...
    if ((IPTDirOK == TRUE) && (rtdmXmlData->OutputStream_enabled == TRUE)
            && (errorCode == NO_ERROR))
    {
        /* Fill m_RtdmSampleArray with samples of data */
        ...
        memcpy (&RTDMStream_ptr->IBufferArray[main_buffer_offset],
                &m_RtdmSampleArray, sizeof(m_RtdmSampleArray));
        ...
        memset (&m_RtdmSampleArray, 0, sizeof(m_RtdmSampleArray));
    }
        ...
        UINT16 *errorCode, RtdmXmlStr *rtdmXmlData)
{
    ...
    if (m_SampleCount == 0)
    {
        /* Allocate memory to store data according to buffer size from .xml file */
        RTDMStream_ptr = (RTDMStream_str *) calloc (
                sizeof(UINT16) + STREAM_HEADER_SIZE + rtdmXmlData->bufferSize,
                sizeof(UINT8));
...
static void Populate_Samples (RtdmXmlStr *rtdmXmlData)
{
    ...
    for (i = 0; i < rtdmXmlData->signal_count; i++)
    {
        ...
        case 23:
            ...
            /*m_RtdmSampleArray.SigValue_23 = m_Interface1Ptr->oPCU_I1.Analog801.ICarSpeed;*/
            temp_car_speed = temp_car_speed + 10;
            m_RtdmSampleArray.SigValue_23 = temp_car_speed;
            break;
