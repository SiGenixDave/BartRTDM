...
int ReadXmlFile(RtdmXmlStr *rtdmXmlData)
{
	...
	if(pStringLocation1 != NULL)
	{
		...
		pStringLocation2 = strstr(pStringLocation1,xml_DataLogFileCfg_enabled);
		if(pStringLocation2 != NULL)
		{
			/* move pointer to TRUE/FALSE */
		...
		pStringLocation2 = strstr(pStringLocation1,xml_numberSamplesBeforeSave);
		if(pStringLocation2 != NULL)
		{
			/* move pointer to id # */
		...
		pStringLocation2 = strstr(pStringLocation1,xml_OutputStreamCfg_enabled);
		if(pStringLocation2 != NULL)
		{
			/* move pointer to TRUE/FALSE */
			pStringLocation2 = pStringLocation2  + 25;
		...
		/* find signal_id */
		while((pStringLocation3 = strstr(pStringLocation3,xml_signal_id)) != NULL)
		{
			/* find signal_id */
			...
			if( (strncmp(temp_array, xml_uint32, 5) == 0) || (strncmp(temp_array, xml_int32, 5) == 0) )
			{
				dataType = 4;
			}
			else if( (strncmp(temp_array, xml_uint16, 5) == 0) || (strncmp(temp_array, xml_int16, 5) == 0) )
			{
				dataType = 2;
			}
